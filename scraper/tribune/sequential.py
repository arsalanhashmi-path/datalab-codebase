...
from tribune.parser import parse_article

logger = logging.getLogger(__name__)

TABLE = "tribune_articles"

def get_max_live_id() -> int | None:
    session = make_session()
    try:
        r = session.get("https://tribune.com.pk", timeout=15)
        ids = [
            int(m)
            for m in re.findall(r"/story/(\d+)", r.text)
        ]
...
def run():
    ...
    start_id = int(override) if override.isdigit() else get_checkpoint("tribune")
    override_max = os.environ.get("OVERRIDE_MAX_ID", "").strip()
    max_id = int(override_max) if override_max.isdigit() else get_max_live_id()

    if not start_id:
        logger.error("Cannot determine start_id — no checkpoint for 'tribune' and no OVERRIDE_START_ID set")
        return
    if not max_id:
        logger.error("Cannot determine max_id — failed to fetch live ID from tribune.com.pk homepage")
        return
    ...
    run_id = start_run("tribune", "sequential")
    ...
    for article_id in range(start_id + 1, max_id + 1):
        ...
        if upsert_article(article, table=TABLE):
            new += 1
            last_id = str(article_id)
            save_checkpoint("tribune", article_id)
